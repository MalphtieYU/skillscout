...
SkillScout 的规则数据是决策模型，而不是永远正确的插件目录。涉及“当前可用、已安装、刚发布、最新”的能力时，它会先查看当前会话，再在可用时核验官方 Codex 信息。当前的发现入口为 **Plugin Directory**；目录可见、安装策略、所需 App 权限和外部数据源权限是彼此独立的检查项。无法核实时只推荐能力类别，绝不编造安装链接、上架状态或权限。
